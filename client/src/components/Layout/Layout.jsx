...
const Layout = ({ children }) => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white text-zinc-900 selection:bg-black selection:text-white transition-colors duration-500">
      <CartToast />
      <Header />
      <main className="flex-grow pt-0 pb-28">
        {children}
      </main>
      {!isCheckout && <Footer />}
      ...
      <a
        ...
        className="fixed bottom-32 right-6 z-[110] h-14 w-14 bg-[#25D366] text-white rounded-full flex items-center justify-center shadow-2xl hover:scale-110 transition-transform duration-300 group"
